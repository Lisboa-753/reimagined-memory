# Resolvendo o problema dos Missionarios e canibais..
# Representação (me, ce, b, md, cd) me -> missionarios na esquerda,
#                                   cd -> canibais na esquerda,
#                                    b -> barco,
...
def isObjective(estado):
    return estado == (0, 0, 'd', 3, 3) #Estado objetivo

# Criar as restrições do problema
def isValid(estado):
    (me, ce, b, md, cd) = estado
    if me > 3 or me < 0 or ce > 3 or ce < 0 or md > 3 or md < 0 or cd > 3 or cd < 0:
        return False
...
# Busca por largura
no     = {"estado": (3, 3, 'e', 0, 0), "pai": None} #Estado Inicial
fila   = []
while not isObjective(no["estado"]):
    filhos = expand(no["estado"])
    for filho in filhos: 
        folha = {"estado": filho, "pai": no} #Armazena o estado e aponta o pai
        fila.append(folha) #Add no final da fila
    no = fila.pop(0) # Remove o primeiro estado da fila (BFS)
